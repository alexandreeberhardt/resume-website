...
const makeData = (): ResumeData => ({
  ...
  sections: [{ id: 'sec-1', type: 'education', title: 'Education', isVisible: true, items: [] }],
  template_id: 'harvard',
})
...
describe('useAutoSize', () => {
  ...
  it('has correct initial state: autoSize=true, recommendedSize="normal", autoSizeLoading=false', () => {
    ...
    const { result } = renderHook(() => useAutoSize({ data: makeData(), setData: mockSetData }))
  ...
  it('updates recommendedSize and calls setData on successful API response', async () => {
    ...
    const data = makeData()
    const { result } = renderHook(() => useAutoSize({ data, setData: mockSetData }))
