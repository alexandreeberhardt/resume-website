...
export default function LeadershipSection({ data, onChange }: LeadershipSectionProps) {
  const addItem = () => {
    onChange([...data, { role: '', place: '', dates: '', highlights: [] }]);
  };
  ...
  const updateItem = (index: number, field: keyof Leadership, value: string | string[]) => {
    const updated = [...data];
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-900">Leadership & Engagement</h2>
        <button
          onClick={addItem}
      ...
      <div className="space-y-6">
        {data.map((lead, index) => (
          <div key={index} className="border border-gray-200 rounded-lg p-4 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Lieu / Organisation</label>
                <input
                  type="text"
                  value={lead.place}
        ...
        {data.length === 0 && (
          <p className="text-gray-500 text-center py-4">Aucun engagement ajouté</p>
        )}
      </div>
    </div>
  );
